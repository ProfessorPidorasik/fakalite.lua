...
local function hookMouse()
    if silentAimHooked or not mouse or not getrawmetatable then
        return
    end
    ...
    local newIndex = function(self, key)
        if self == mouse and silentAimEnabled and targetInfo.part and targetInfo.position then
            if key == "Hit" then
            ...
            elseif key == "X" or key == "Y" then
                if Camera then
                    ...
                    if onScreen then
                        return (key == "X") and viewportPoint.X or viewportPoint.Y
                    end
                end
            end
        end
...
local function applyAntiAim()
    ...
    local yaw = math.rad(antiAimYaw)
    if antiAimMode == "spin" then
        yaw = math.rad((tick() * 360) % 360) + math.rad(antiAimYaw)
    elseif antiAimMode == "jitter" then
        yaw = math.rad(antiAimYaw + math.random(-antiAimJitter, antiAimJitter))
    end
...
registerBinding("visuals.esp.enabled", function(value)
    ...
    for player, entry in pairs(playerVisuals) do
        if not espEnabled and entry.esp then
            entry.esp.Enabled = false
        else
            ensureEsp(entry, player)
        end
...
registerBinding("visuals.chams.enabled", function(value)
    ...
    for player, entry in pairs(playerVisuals) do
        if not chamsEnabled and entry.highlight then
            entry.highlight.Enabled = false
        else
            ensureHighlight(entry, player)
        end
